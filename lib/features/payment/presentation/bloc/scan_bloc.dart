...
class ScanBloc extends Bloc<ScanEvent, ScanState> {
  ...
  Future<void> _onQrDetected(
    ...
    if (result == null) {
      emit(
        state.copyWith(status: ScanStatus.error, error: 'Code QR invalide.'),
      );
      return;
    }
    ...
    either.fold(
      ...
      (machine) {
        ...
        if (availabilityError != null) {
          emit(
            state.copyWith(status: ScanStatus.error, error: availabilityError),
          );
        } else {
          emit(state.copyWith(status: ScanStatus.success, machine: machine));
        }
      },
    );
  }
